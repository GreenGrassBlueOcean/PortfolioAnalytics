...
extract_risk <- function(R, w, ES_alpha = 0.05, CSM_alpha = 0.05, EQS_alpha = 0.05, moment_setting = NULL){
  ...
  res_es <- CVXR::psolve(p_es, solver = "SCS")
  res$ES <- res_es$value
  
  ## CSM
  obj_CSM <- zeta + (1/(sqrt(T)*CSM_alpha)) * CVXR::p_norm(z, p=2)
  con_CSM <- list(z >= 0, z >= -X %*% w - zeta)
  p_CSM <- CVXR::Problem(CVXR::Minimize(obj_CSM), constraints = con_CSM)
  res_CSM <- CVXR::psolve(p_CSM, solver = "SCS")
  ...
  res_EQS <- CVXR::psolve(p_EQS, solver = "SCS")
