...
constrained_objective_v1 <- function(w, R, constraints, ..., trace=FALSE, normalize=TRUE, storage=FALSE, penalty=1e4, storage_env=NULL)
{
    ...
    # Retrieve accumulated storage from per-call local environment (if active)
    store_output <- if (!is.null(storage_env)) {
      tryCatch(get(".objectivestorage", envir = storage_env, inherits = FALSE),
               error = function(e) NULL)
    } else {
      NULL
    }
    storage <- !is.null(store_output)
...
#                     max_diff<-max(tmp_measure[[2]]-(sum(tmp_measure[[2]])/length(tmp_measure[[2]]))) #second element is the contribution in absolute terms
    ...
    if(isTRUE(verbose)) {
        print('weights: ') # nolint: print_linter.
        print(paste(w,' ')) # nolint: print_linter.
        print(paste("output of objective function",out)) # nolint: print_linter.
        print(unlist(tmp_return)) # nolint: print_linter.
    }
    ...
    if (isTRUE(storage)){
        ...
        assign('.objectivestorage', store_output, envir = storage_env)
    }
    if(!isTRUE(trace)){
        return(out)
    } else {
...
#'
#' Estimates an appropriate penalty value by evaluating the objective function
#' on a small pilot set of random portfolios with \code{penalty=0} (zeroing out
#' constraint penalty contributions). The returned penalty is scaled relative
#' to the observed magnitude of objective values, ensuring constraint violations
#' dominate the objective without overwhelming numerical precision.
#'
#' The default penalty of \code{1e4} used by \code{\link{constrained_objective}}
#' is arbitrary and can cause problems when the objective function returns values
#' on a very different scale. For small-scale objectives (e.g., daily variance
#' ~1e-4), the penalty dwarfs the objective, causing stochastic solvers to ignore
#' it. For large-scale objectives, the penalty may be insufficient.
#'
#' @param R an xts, vector, matrix, data frame, timeSeries or zoo object of
#'   asset returns
#' @param portfolio an object of class \code{portfolio} specifying the
#'   constraints and objectives for the optimization
#' @param env optional list of pre-computed moments (as returned by
#'   \code{momentFUN}). If \code{NULL}, moments are recomputed internally.
#' @param \dots additional arguments (currently unused, reserved for future use)
#' @param n_pilot number of pilot random portfolios to generate (default 20)
#' @param scaling_factor multiplier applied to the observed objective scale
#'   (default 10000). A value of 10000 ensures that a 1\% constraint violation
#'   produces a penalty term roughly 100x the typical objective value.
#' @param min_penalty floor value for the returned penalty (default 1)
#' @return a single positive numeric penalty value
#' @seealso \code{\link{constrained_objective}}, \code{\link{optimize.portfolio}}
#' @export
calibrate_penalty <- function(R, portfolio, env = NULL, ...,
  ...
  if (inherits(pilot_rp, "try-error") || NROW(pilot_rp) < 3) {
    return(1e4)
  }

  # penalty=0 zeros out all constraint penalty terms, leaving pure objective
  pilot_vals <- apply(pilot_rp, 1, function(w) {
    val <- try(
      constrained_objective(w = w, R = R, portfolio = portfolio,
                            penalty = 0, normalize = TRUE, env = env),
      silent = TRUE
    )
    if (inherits(val, "try-error") || !is.finite(val)) NA_real_ else val
  })
  pilot_vals <- pilot_vals[is.finite(pilot_vals)]

  if (length(pilot_vals) < 3) return(1e4)

  obj_scale <- median(abs(pilot_vals))
  if (is.na(obj_scale) || obj_scale == 0) return(1e4)
...
#'   objective function magnitude.
#' @param storage_env an environment for per-call objective accumulation used
#'   by the DEoptim solver when \code{trace=TRUE}. Not typically user-called.
#'   Defaults to \code{NULL} (disabled).
#' @seealso \code{\link{constraint}}, \code{\link{objective}}, \code{\link[DEoptim]{DEoptim.control}}, \code{\link{calibrate_penalty}}
...
constrained_objective <- constrained_objective_v2 <- function(w, R, portfolio, ..., trace=FALSE, normalize=TRUE, storage=FALSE, env=NULL, penalty=1e4, storage_env=NULL)
{
  if (ncol(R) > length(w)) {
    R <- R[ ,1:length(w)]
  }
