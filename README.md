...
- **`normalize` is settable on the DEoptim path** — `constrained_objective()`'s `normalize` decides whether a candidate that misses the leverage band is rescaled onto it or penalised instead. `solve_deoptim()` hard-wired it to `FALSE` while `constrained_objective()`'s own default is `TRUE`. The difference is not cosmetic: on a narrow band (the `0.98`–`1.02` of a typical `full_investment` constraint) almost every mutation of a many-asset weight vector lands outside it, so nearly every trial loses to its parent and the population can stop moving after a handful of generations. It remains `FALSE` by default so existing results stay bit-identical; pass `normalize = TRUE` through `...` to measure the alternative without patching an install.
- **Moments cross to the workers once, not every generation** — DEoptim's cluster path evaluates a generation with `parApply(cl, pop, 1, fn, ...)`, which re-serializes `FUN` and every `...` argument to every worker on each call; `solve_deoptim()` passed `env = moments` that way. For CRRA the moment list carries the `m4` co-moment tensor, `N x N^3`: at 60 assets that is 103.7 MB on its own, so an 18-worker cluster moved ~1.87 GB per generation and ~370 GB over a 200-generation run, for a value `momentFUN` computes once. Measured on 4 local workers with a 98.9 MB payload the overhead was 0.60–0.75 s **per generation** and linear in `itermax` — transport, not a setup cost, and not recomputation either (`optimize.portfolio()` calls `momentFUN` once and `constrained_objective()` uses the `env` it is handed). The moments now cross in a single `clusterCall()` into a namespace-level cache that the objective reads. Results are unchanged; `moments_cache = FALSE` restores the previous transport for comparison. Because workers resolve namespace closures against *their own* installed PortfolioAnalytics rather than the master's, `solve_deoptim()` probes for cache support first and falls back to the old transport with a warning when a worker is running an older build — a version skew costs throughput, not the run.
