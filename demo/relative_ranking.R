...
# data("DailyReturns")
R <- edhec[,1:4]
# R <- DailyReturns[, which(colnames(DailyReturns) %in% c("VYM.NB", "KWEB.NB", "QQQ.NB", "VDEM.L"))]
funds <- colnames(R)
...
init.portf <- add.objective(portfolio=init.portf, type="risk", name="ETL"
                            , arguments=list(p=0.9, method="gaussian",  clean="boudt"))
init.portf <- add.objective(portfolio=init.portf, type="return", name="mean")

#' #' Here we express views on the relative rank of the asset returns.
#' #' E{ R[,2] < R[,3] < R[,1] < R[,4] }
#' asset.rank <- c(2, 3, 1, 4) #seq(from = 1 , to =ncol(R)) # 
#' 
...
#' previous n periods.
moment.ranking <- function(R, n=1, momentum=FALSE, method=c("meucci", "ac")){
  ...
  switch(method,
         ...
         ac = {
           ...
           # Sample estimate for second moment
           moments$sigma <- cov(R)
           # moments$m3 <- PerformanceAnalytics::M3.MM(R)
           # moments$m4 <- PerformanceAnalytics::M4.MM(R)
         }
  )
  print(dput(moments))
  return(moments)
...
opt.bt.ac <- optimize.portfolio.rebalancing(R, init.portf, 
                                            optimize_method="DEoptim",  #ramdom, 
                                            rebalance_on="quarters", 
                                            training_period=ceiling(252/2),
                                            steptol = 5*ncol(R),
                                            SearchSize = 100000 * ncol(R),
                                            momentFUN="moment.ranking",
                                            n=ceiling(252/8),
                                            traceDE = 20,
                                            momentum=TRUE,
...
ret <- cbind(ret.ac, ret.sample) #ret.meucci
colnames(ret) <- c( "ac.rank", "sample") #"meucci.rank"
