...
test_that("minStdDev.lo.ROI objective measure StdDev is positive and plausible", {
  val <- as.numeric(extractObjectiveMeasures(minStdDev.lo.ROI)$StdDev)
...
test_that("minStdDev.box.ROI objective measure StdDev is positive and plausible", {
  val <- as.numeric(extractObjectiveMeasures(minStdDev.box.ROI)$StdDev)
  expect_true(is.numeric(val) && val > 0 && val < 0.05)
})
