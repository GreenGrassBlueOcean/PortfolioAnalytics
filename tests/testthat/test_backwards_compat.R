...
test_that("backwards_compat.R runs succesfully", {
  # v1 functions now emit deprecation warnings; suppress them here since

  # the demo is specifically exercising the deprecated v1 API.
  expect_error(suppressWarnings(source(system.file("demo/backwards_compat.R", package="PortfolioAnalytics"))), NA)
})
...
test_that("ROI returns a positive, plausible objective measure for mean", {
  val <- as.numeric(extractObjectiveMeasures(optroiv1)$mean)
