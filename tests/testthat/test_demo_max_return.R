...
test_that("demo_max_return.R runs succesfully", {
  # Use pdf(NULL) to prevent "figure margins too large" errors in non-interactive sessions
  pdf(NULL)
...
test_that("maxret.lo.ROI objective measure mean is positive and plausible", {
  val <- as.numeric(extractObjectiveMeasures(maxret.lo.ROI)$mean)
...
test_that("maxret.box.ROI objective measure mean is positive and plausible", {
  val <- as.numeric(extractObjectiveMeasures(maxret.box.ROI)$mean)
  expect_true(is.numeric(val) && val > 0 && val < 0.05)
})
