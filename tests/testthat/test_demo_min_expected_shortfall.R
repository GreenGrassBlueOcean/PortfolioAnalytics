...
test_that("minES.lo.ROI objective measure ES is positive and plausible", {
  val <- as.numeric(extractObjectiveMeasures(minES.lo.ROI)$ES)
...
test_that("minES.box.ROI objective measure ES is positive and plausible", {
  val <- as.numeric(extractObjectiveMeasures(minES.box.ROI)$ES)
  expect_true(is.numeric(val) && val > 0 && val < 0.10)
})
...
test_that("minES.box.DE min box constraints are not violated", 
          { expect_true(all(extractWeights(minES.box.DE) >= minES.box.DE$portfolio$constraints[[2]]$min - 1e-4)) })
